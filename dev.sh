...
# --- Font check (PDF export requires book fonts for lualatex) ---
# All book fonts are managed by `make install-fonts` which installs to
# /usr/local/share/fonts/photo-sorter using scripts/install-fonts.sh — the
# same script the Docker build runs. We probe for one canonical sentinel
# file (Lato-Regular) to detect a fresh checkout and remind the user.
if [ ! -f "/usr/local/share/fonts/photo-sorter/truetype/lato/Lato-Regular.ttf" ]; then
  echo "==> Book fonts not installed. Run: make install-fonts" >&2
  echo "    (PDF export will fail until they are installed.)" >&2
fi
