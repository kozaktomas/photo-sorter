...
- **Inline rename** - Double-click (or F2 with the title focused) on any album, label, book, or smart-album title — in both list cards and detail headers — to switch into an `<input>` editor; Enter or blur commits, Esc cancels. Editors/admins only; viewers see the static text without the edit affordance. The new value renders optimistically; failed PUTs roll back and surface a toast. Other fields (description, location, etc.) still go through the existing Edit modal.
