...
import { FolderOpen, Tags, Sparkles, Home, Users, Images, Camera, Maximize2, AlertTriangle, Type, ShieldCheck, Cpu, ChevronDown, LogOut, Copy, FolderSearch, BookOpen, Upload } from 'lucide-react';
  ...
  useEffect(() => {
    getConfig().then(cfg => {
      if (cfg.version && cfg.version !== 'dev') {
        setVersionLabel(cfg.version);
      } else if (cfg.commit_sha && cfg.commit_sha !== 'unknown') {
        setVersionLabel(cfg.commit_sha);
      } else {
        setVersionLabel('dev');
      }
    }).catch(() => {
          ...
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center space-x-2">
              <a
                ...
                className="flex items-center gap-1.5 px-2 py-2 rounded-md text-slate-300 hover:bg-slate-700 hover:text-white transition-colors"
                title="GitHub"
              >
                ...
                {versionLabel && (
                  <span className="text-xs font-mono text-slate-400">{versionLabel}</span>
                )}
              </a>
              <LanguageSwitcher />
              <button
