...
export const Button = forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className = '', variant = 'primary', size = 'md', isLoading, accentColor, children, disabled, ...props }, ref) => {
    ...
    if (variant === 'accent' && accentColor) {
      ...
      variantStyles.accent = `${c.buttonBg} text-white ${c.buttonHover} focus-visible:ring-slate-500`;
    } else {
    ...
    return (
      ...
      >
        {isLoading && (
          <svg className={`animate-spin -ml-1 mr-2 ${spinnerSize[size]}`} fill="none" viewBox="0 0 24 24">
