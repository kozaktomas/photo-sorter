...
export function useSSE(url: string | null, options: SSEOptions = {}) {
  ...
  useEffect(() => {
    ...
    ['status', 'started', 'progress', 'photos_counted', 'filtering_done', 'completed', 'job_error', 'cancelled', 'upload_progress', 'processing_upload', 'detecting_photos', 'applying_labels', 'applying_albums', 'adding_to_book', 'process_progress', 'near_duplicates'].forEach((type) => {
