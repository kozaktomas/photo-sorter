...
import { Check, Loader2, UserPlus, RefreshCw, UserX, X } from 'lucide-react';
...
interface FaceAssignmentPanelProps {
  ...
  onStartReassign: (faceIndex: number) => void;
  onCancelReassign: () => void;
  onUnassign: (face: PhotoFace) => void;
}
...
export function FaceAssignmentPanel({
  ...
  const isApplying = applyingFace === selectedFace.face_index;

  // Filter out the current person from suggestions when reassigning
  const displaySuggestions = isReassigning
    ? selectedFace.suggestions.filter(s => s.person_name !== selectedFace.marker_name)
    : selectedFace.suggestions;

  const renderSuggestionsAndManualInput = () => (
        ...
        <div className="space-y-2">
          ...
          <div className="flex gap-2">
            <Button
              ...
              disabled={!manualName.trim() || isApplying}
              className="flex-1"
            >
              {isApplying ? (
                <Loader2 className="h-4 w-4 animate-spin" />
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-sm text-slate-300">
                {t('pages:photoDetail.reassigningFrom', { name: selectedFace.marker_name })}
              </span>
              <button
            ...
            {renderSuggestionsAndManualInput()}
          </div>
        ) : (
          /* Default assigned view with reassign/unassign buttons */
          <div className="space-y-3">
            ...
            <div className="flex gap-2">
              <Button
                size="sm"
                variant="ghost"
                onClick={() => onStartReassign(selectedFace.face_index)}
                disabled={isApplying}
                className="flex-1"
              >
                <RefreshCw className="h-3.5 w-3.5 mr-1" />
                {t('pages:photoDetail.reassign')}
              </Button>
              <Button
                size="sm"
                variant="ghost"
                onClick={() => onUnassign(selectedFace)}
                disabled={isApplying}
