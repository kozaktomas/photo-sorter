...
- `GET /api/v1/public/share/{slug}/` - Public metadata (no auth). Returns `{ has_password, expires_at, album: { title, photo_count, cover_thumb_url } }`. 404 when unknown; 410 when expired. Album payload is hidden until the recipient verifies the password. `photo_count` is computed from a filter that excludes archived and private rows (matching the listing endpoint); `cover_thumb_url` is suppressed when the album's cover photo is archived or private.
- `POST /api/v1/public/share/{slug}/verify` - Public password check (no auth). Sets a 24h `share_<slug>` HttpOnly cookie with `Path=/api/v1/public/share/<slug>/` (trailing slash keeps the cookie from bleeding onto sibling slugs that share a prefix) on success. Rate-limited to 10 attempts per (IP, slug) per 5 minutes (429 + `Retry-After`); the IP comes from `r.RemoteAddr` after chi's `RealIP` middleware, so an unproxied attacker cannot rotate `X-Forwarded-For` to dodge the bucket.
- `GET /api/v1/public/share/{slug}/photos` - Paginated public photo listing (`limit` capped at 1000, default 200). Archived and private photos are filtered out server-side. Requires the share cookie when password-protected.
- `GET /api/v1/public/share/{slug}/photos/{photo_uid}/thumb/{size}` - Public thumbnail stream (no app auth; share cookie when protected). Photo must belong to the linked album AND must not be archived or marked private — even a known-member UID is hidden if the row is archived/private.
- `GET /api/v1/public/share/{slug}/photos/{photo_uid}/download` - Public original download with Range support. Same archived/private gating as the thumbnail endpoint.
...
- `GET /api/v1/photos/{uid}/download` - Stream the original primary file as an attachment with `Range` support. When the photo has stored non-destructive edits (`photo_edits` row), the default response is a freshly-rendered JPEG (decode original → apply crop/rotate/brightness/contrast → encode at quality 92). Pass `?original=true` to bypass rendering and stream the pristine file.
...
- `GET /api/v1/audit-log` - List audit log entries (admin only). Records every successful mutating action plus `login_failed` / `share_link_password_failed`. Filter params: `user_uid`, `action`, `entity_type`, `entity_uid`, `since` (RFC3339), `until` (RFC3339), `limit` (default 50, max 200 — silently clamped), `offset`. Envelope `{ entries, total, limit, offset }`; each entry includes the joined `user_username` (empty string when actor was anonymous or has since been deleted — the FK uses `ON DELETE SET NULL`). Batch operations record a single row with `metadata.count` rather than one row per item, so high-volume bulk ops do not blow up the table. Backed by the new `internal/audit/` package + the `audit_log` table (migration 042).
